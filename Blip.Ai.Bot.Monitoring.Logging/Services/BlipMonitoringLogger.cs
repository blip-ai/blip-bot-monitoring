...
public class BlipMonitoringLogger : IBlipLogger
{
    ...
    public BlipMonitoringLogger(LoggingOptions options)
    {
        ...
        {
            var credentials = !string.IsNullOrWhiteSpace(options.Grafana.LokiLogin) &&
                              !string.IsNullOrWhiteSpace(options.Grafana.LokiPassword)
                ? new LokiCredentials()
                {
                    Login = options.Grafana.LokiLogin, Password = options.Grafana.LokiPassword
                }
                : null;
...
#pragma warning restore CA2000
            ...
            if (!string.IsNullOrWhiteSpace(options.Grafana.LokiHeaderName) &&
                !string.IsNullOrWhiteSpace(options.Grafana.LokiHeaderValue))
            {
                httpClient.DefaultRequestHeaders.Add(options.Grafana.LokiHeaderName,
            ...
            if (!string.IsNullOrWhiteSpace(options.Grafana.LokiLabels))
            {
                ...
                {
                    labels.AddRange(labelsFromConfig.Select(x => new LokiLabel
                        { Key = x.Key, Value = x.Value }));
                }
            }

            loggerConfig
                .WriteTo.GrafanaLoki(options.Grafana.LokiUri,
                    labels: labels,
    ...
    private void Log(LogCategory category, LogInput input, string? ex = null,
        [CallerMemberName] string caller = "")
    {
        ...
        var level = category == LogCategory.ErrorEvents
            ? LogEventLevel.Error
